...
if ( ! class_exists( 'RWMB_Map_extended_Field' ) )
{
	class RWMB_Map_extended_Field extends RWMB_Field
	{
		/**
		...
		static function html( $meta, $field )
		{
			...
			if(isset($_GET['post']) && !empty($_GET['post']))
			{
				$post_id = $_GET['post'];
				if(!empty($field['std'])){
					list( $latitude, $longitude, $zoom ) = explode( ',', $meta . ',,' );

					update_post_meta($post_id, 'woocommerce_store_locator_lat', $latitude);
					update_post_meta($post_id, 'woocommerce_store_locator_lng', $longitude);
				}
			}
